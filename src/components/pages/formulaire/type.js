...
export default function Type({className, onPrev, onNext, ...props}) {
    const btnClass = "w-80 flex justify-center bg-white text-text items-center gap-4 rounded-lg border border-gray-400 px-16 py-10 shadow-xl"
    ...
    return (
        <div className={`flex flex-col items-center ${className}`} {...props}>
            ...
            <div className="flex flex-wrap justify-center gap-10 text-xl my-16">
                <button onClick={() => onNext({owner: false})} className={`${btnClass} ${btnClassHover}`}><PiKeyFill size={32} /><span>Locataire</span></button>
                <button onClick={() => onNext({owner: true})} className={`${btnClass} ${btnClassHover}`}><BsHouseDoorFill size={32} /><span>Propriétaire</span></button>
            </div>
        </div>
    )
}