...
export default function Partners() {
    return (
        <section className="relative py-10 !mb-40">
            ...
            <div className="absolute flex bg-third">
                { Array(3).fill().map((_, index) => (
                    <div key={index} className="flex animate-amine">
                    {images.map((image, index) => (
                        <div key={index} className="m-5 w-20 h-20">
                            <img src={image} alt="partner" className="w-full h-full" />
                        </div>
                    ))}
                </div>
                ))
